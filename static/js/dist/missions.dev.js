...
$('a.open-chat').click(function (e) {
  ...
  $(mission).find(".msg-input").focus(); // Cancel the default <a> action

  e.preventDefault();
  socket.emit('mission_msg_read', {
    msg_id: messageId
  });
});
$('a.close-chat').click(function (e) {
  ...
  $(mission).find(".close-chat").hide(); // Cancel the default action
...
socket.on('change_status', function (details) {
  ...
  if (details["status"] == "1") {
    ...
    $(left).width($(timeline).find(".lwidth").val()); // If mission overdue
...
socket.on('edit_mission', function (details) {
  console.log("mission edited", details); // Edit mission details by manager
...
socket.on('delete_msg', function (details) {// Delete mission
...
function toggleMissionUrgent(missionId) {
  $("#flag-" + missionId).toggleClass("urgent");
  socket.emit('toggle_mission_urgent', {
    mission_id: missionId
  });
}