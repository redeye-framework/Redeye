...
function openChat(this_obj){
    ...
    if (groupName.length > 8) {
        $($(this_obj).find("span")).css("font-size", "10px")
    } else {
        $($(this_obj).find("span")).css("font-size", "18px")
    };

    var to = this_obj.querySelectorAll("input")
    var to_type = to[0].value;
    var to_id = to[1].value;
    // init the current personal
    document.getElementById("to").value = to_type;
    document.getElementById("to-user-id").value = to_id;
    // load relevent messages related to that person
    if(to_type == "All"){
        ...
        if(to_type == "All"){
            ...
            if(group_id == to_id){
                if(file_path && file_path.includes("/")){
                    ...
                    msg = {"leave_group":'<div class="message user-left" style=float:left><span class="username">' + sender_user_name + '</span> has left the group</div><br>'}
                }
                else{
                    msg = build_obj(sender_type + "\\" + sender_user_name,date,content);
                }
...
function build_obj_with_file(sender,date,content,file_path){
    return {'sender': sender,'date':date, 'content':content, 'file_path': file_path}
}


function download_file(path){
    $.post(Flask.url_for('download_chat_file', {file_name: path}));
}
...
function new_chat(user, link){
    ...
    socket.emit("new_msg", {msg: "", type:type, userid:recive_user_id}); // Send empty message.
...
$("#msg-send-btn").click(function new_msg(){
    ...
    if (!file){
        socket.emit("new_msg", {msg: msg, type:type, userid:userid});
    }
    else{
        socket.emit("new_msg", {msg: msg, type:type, userid:userid,filename:file.name ,file:file});
    }
...
function add_new_msg(msg, type,recive_username, sender_username, recive_id, sender_id){
    ...
    var sector_id = document.getElementsByClassName("current")[0].querySelectorAll("input")[1].value; // sender userid
    if(type == "All"){ // all msg
        if(sector == "All"){  // if current tab of user is All ==> add msg
            document.getElementById("messages").innerHTML += new_msg;
        }
        //else{ // else push it to array
        new_msgs.push({"msg":msg,type:"All"});
        //}
    }
    else if(type == "groups"){ // group msg
        if(recive_id == sector_id){ // if the group_id of msg == id of current tab ==> add msg
            document.getElementById("messages").innerHTML += new_msg;
        }
        //else{  // else push it to array
        new_msgs.push({"msg":msg,type:"Group",id:recive_id}); // [msg, "group", group_id]
        //}
    }
    else{ // private msg
        var current_id = document.getElementById("current_user_id").value;
        if (sender_id == current_id){ // if im the sender
        ...
        //else{ // else push it to array
        console.log("in else")
        new_msgs.push({"msg":msg,type:"Private",id:sender_id}); // [msg,"private",sender_id]
        //}
    }
}
...
function change_profile_pic(){
    ...
    socket.emit("update_profile_pic", {profile_pic: profile_pic, ext: ext});
...
function hide_chats(user,type, obj){
    ...
    socket.emit("change_chat_state", {user_id: recive_user_id, type: type, action:action}); // Send empty message.
}
    ...
    if (owner_type == "blue" && owner_id == user_id) {
        ...
        $(".group-name-edit").click(function (){
            ...
            $(".group-name-input").focusout(function(){
                ...
                socket.emit("edit_group_name", {group_id: group_id, new_group_name: newName})
            })
        })
    } else {
        $(".delete-group").hide()
...
function saveNewGroup() {
    ...
    socket.emit("add_group",{group_name:group_name,users:users})
    location.reload()
}
...
function deleteGroup() {
    ...
    if (!confirm('This will Permanently delete this group. Do you wish to continue?')) {
        ...
        socket.emit("delete_group", {group_id:group_id})
    }
}
...
function change_group_pic(){
    ...
    var ext = profile_pic.name.split(".").pop();
    if (id){
        socket.emit("update_group_profile_pic", {profile_pic: profile_pic, ext: ext, group_id: id});
        location.href = '/chats?settings=1';
    } else {
...
$(".update-members").click(function() {
    ...
    socket.emit("update_group_users", {group_id: group_id, users: members});
})
...
socket.on('on_leave_group', function(details) {
    ...
    user_left_group(new_msg,details["group_id"])
...
socket.on('on_add_group', function(details) {
    console.log("Added new group !", details);
    // Do actions here to add the group in front end ! (in shared groups)
    // details["group_info"]
});

socket.on('on_update_group_users', function(details) {
    console.log("group_updated ! current users ==> ",details)
    //$(".selected-group-members").val(details["users"])
...
socket.on('on_edit_group_name', function(details) {
    ...
    var newName = details["new_group_name"]

    // Change name on chats settings
    $("#group-" + group_id).find(".name").text(newName)

    // Change name on group chat
